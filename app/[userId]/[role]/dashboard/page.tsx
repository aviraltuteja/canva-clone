...
export default function Dashboard(): React.ReactElement {
  ...
  const userDesigns = useAtomValue(ownedDesignsAtom);
  ...
  if (loading) {
    return (
      <div className="p-8 text-lg text-primary">Loading your dashboard...</div>
    );
  }
    ...
    <div className="p-8 flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-8">
        <div id="designs" className="grid grid-cols-6 gap-4">
          <AddDesignIcon />
          {userDesigns.map((design, key) => (
            <div key={key}>
              <DesignInfoIcon design={design} />
            </div>
          ))}
        </div>
      </div>
